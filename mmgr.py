...
if __name__=='__main__':
    ...
    #dataset
    train_dataset=PoiImageAugDataset(path=args.data_path,mode='train',poi_f='poi12')
    train_loader=torch_geometric.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,num_workers=12)
    ...
    for epoch in range(1,args.total_epoch+1):
        ...
        for step,data in enumerate(train_loader):
            ...
            if step % 50 == 0:
                print(f"epoch [{epoch}/{args.total_epoch}]\t Step [{step}/{len(train_loader)}]\t Loss1: {loss_step1/50}\t Loss2: {loss_step2/50}")
                record_file = open(record_path, 'a')
                record_file.write(f"Step [{step}/{len(train_loader)}]\t Loss1: {loss_step1 / 50}\t Loss2: {loss_step2/50}\n")
                record_file.close()
                loss_step1=0
                loss_step2=0
